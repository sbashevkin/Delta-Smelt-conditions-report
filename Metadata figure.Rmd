...
p<-ggplot(filter(d, !is.na(Stations)), aes(x=Region, y=Stations, fill=Source))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(angle=45, hjust=1), panel.grid=element_blank(), strip.background = element_blank(), text=element_text(size=12), plot.margin = margin(0,0,0,12), strip.text.y = element_text(angle=0, hjust=0))

p

ggsave("Metadata figure.png", p, device="png", width=9, height=7)
```

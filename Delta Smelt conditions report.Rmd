...
```{r Secchi, fig.height=7, fig.cap="Figure 7. Mean Fall Secchi depth from the FMWT, EMP, and EDSM surveys."}
WQ$Secchi
```
...
```{r Salinity, fig.height=7, fig.cap="Figure 8. Mean Fall salinity from the FMWT and EMP suveys."}
WQ$Salinity
```
...
```{r Chlorophyll, fig.height=3.8, fig.cap="Figure 9. Mean summer chlorophyll concentrations from EMP. No data are available from the Sac Deep Water Shipping Channel or Cache/Slough/Liberty Island regions."}
WQ$Chlorophyll
```
...
```{r Phytoplankton, fig.cap="Figure 10. Summer phytoplankton community composition from EMP. Canobacteria are presented in Fig. 11. No data are available from the Sac Deep Water Shipping Channel or Cache/Slough/Liberty Island regions. Data are only shown from 2008 - 2018 to reflect the timeframe of consistent sample processing methodology."}
PH$Phytoplankton
```
...
```{r Cyanobacteria, fig.height=3.6, fig.cap="Figure 11. Summer cyanobacteria abundance from EMP. No data are available from the Sac Deep Water Shipping Channel or Cache/Slough/Liberty Island regions. Data are only shown from 2008 - 2018 to reflect the timeframe of consistent sample processing methodology."}
PH$Cyanobacteria
```
...
```{r Microcystis, fig.height=4.6, fig.cap="Figure 12. Summer Microcystis bloom intensity from EMP and STN. Microcystis bloom presence and intensity are measured on a qualitative scale from 1-5."}
WQ$Microcystis
```
...
```{r Zooplankton, fig.height=3.9, fig.cap="Figure 13. Fall zooplankton biomass and composition from EMP. No data are available from the Sac Deep Water Shipping Channel or Cache/Slough/Liberty Island regions."}
...
```{r Bivalves, fig.height=4.1, fig.cap="Figure 14. Fall abundance of two invasive bivalve species (the overbite clam *Potamocorbula amurensis* and the freshwater clam *Corbicula fluminea*). No data are available from the Sac Deep Water Shipping Channel or Cache/Slough/Liberty Island regions."}
...
```{r Delta_Smelt_EDSM, fig.height=5, fig.cap="Figure 16. Monthly Delta smelt abundance estimates from the Enhanced Delta Smelt Montoring survey (EDSM). No Delta Smelt were collected from the Southern Delta."}
FISH$EDSM
```

EDSM is a new survey that calculates Delta Smelt abundance estimates throughout the year. __*Note that the y-axis is on the log scale and the x axis starts in mid 2017. The y axis starts at the lowest estimated detection value; all values below the limit are 0s.*__ 
...
```{r Delta_Smelt_photo, echo=FALSE, fig.cap="Juvenile Delta Smelt Dale Kolke / DWR", out.width="55%"}
...
```{r Delta_map, echo=FALSE, out.height="75%"}
knitr::include_graphics("Figures/map.png")
```


```{r Metadata_figure, echo=FALSE, warning = FALSE, message=FALSE, error=FALSE, fig.height=9}
DSCMetadater(Data)
```
...
```{r Cyanobacteria, fig.height=3.6}
PH$Cyanobacteria
```
...
```{r Delta_smelt_IEP, fig.height=6}
FISH$IEP
```
...
```{r Delta_Smelt_EDSM, fig.height=5}
FISH$EDSM
```
